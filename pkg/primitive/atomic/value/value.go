...
type Value[V any] interface {
	...
	// Set sets the value
	Set(ctx context.Context, value V, opts ...SetOption) (atomic.Versioned[V], error)

	// Update updates the value
	Update(ctx context.Context, value V, opts ...UpdateOption) (atomic.Versioned[V], error)

	// Get gets the value
	Get(ctx context.Context, opts ...GetOption) (atomic.Versioned[V], error)
...
type ValueStream[V any] stream.Stream[atomic.Versioned[V]]
...
type atomicValuePrimitive[V any] struct {
	primitive.Primitive
	client valuev1.AtomicValueClient
	codec  generic.Codec[V]
}

func (m *atomicValuePrimitive[V]) Set(ctx context.Context, value V, opts ...SetOption) (atomic.Versioned[V], error) {
	bytes, err := m.codec.Encode(value)
	if err != nil {
		return atomic.Versioned[V]{}, errors.NewInvalid("value encoding failed", err)
	}
	...
	if err != nil {
		return atomic.Versioned[V]{}, errors.FromProto(err)
	}
	for i := range opts {
...
func (m *atomicValuePrimitive[V]) Update(ctx context.Context, value V, opts ...UpdateOption) (atomic.Versioned[V], error) {
	bytes, err := m.codec.Encode(value)
	if err != nil {
		return atomic.Versioned[V]{}, errors.NewInvalid("value encoding failed", err)
	}
...
func (m *atomicValuePrimitive[V]) Get(ctx context.Context, opts ...GetOption) (atomic.Versioned[V], error) {
	...
	if err != nil {
		return atomic.Versioned[V]{}, errors.NewInvalid("value decoding failed", err)
	}
	return atomic.Versioned[V]{
		Version: atomic.Version(response.Value.Version),
		Value:   value,
	}, nil
}
...
func (m *atomicValuePrimitive[V]) Watch(ctx context.Context) (ValueStream[V], error) {
	...
	ch := make(chan stream.Result[atomic.Versioned[V]])
	go func() {
		defer close(ch)
		for {
			...
			if err != nil {
				...
				ch <- stream.Result[atomic.Versioned[V]]{
					Value: atomic.Versioned[V]{
						Version: atomic.Version(response.Value.Version),
						Value:   value,
					},
	...
	return stream.NewChannelStream[atomic.Versioned[V]](ch), nil
}
			...
			case *valuev1.Event_Created_:
				...
				ch <- stream.Result[Event[V]]{
					Value: &Created[V]{
						...
						Value: atomic.Versioned[V]{
							Version: atomic.Version(e.Created.Value.Version),
							Value:   value,
						},
					},
				}
			case *valuev1.Event_Updated_:
				...
				ch <- stream.Result[Event[V]]{
					Value: &Updated[V]{
						...
						OldValue: atomic.Versioned[V]{
							Version: atomic.Version(e.Updated.PrevValue.Version),
							Value:   oldValue,
						},
					},
				}
