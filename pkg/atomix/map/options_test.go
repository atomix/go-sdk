...
func TestOptions(t *testing.T) {
	...
	assert.Equal(t, uint64(0), putRequest.Entry.Key.ObjectMeta.Revision.Num)
	IfMatch(meta.ObjectMeta{Revision: 1}).beforePut(putRequest)
	assert.Equal(t, uint64(1), putRequest.Entry.Key.ObjectMeta.Revision.Num)

	removeRequest := &api.RemoveRequest{}
	assert.Equal(t, uint64(0), removeRequest.Key.ObjectMeta.Revision.Num)
	IfMatch(meta.ObjectMeta{Revision: 2}).beforeRemove(removeRequest)
	assert.Equal(t, uint64(2), removeRequest.Key.ObjectMeta.Revision.Num)
